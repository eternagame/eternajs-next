...
import type SecStruct from './SecStruct';
...
export default class Sequence {
  ...
  public hasCut(from: number, to?: number): boolean {
    if (to === undefined) {
      to = from; // eslint-disable-line no-param-reassign
    }
    return this.baseArray.slice(from, to + 1).some((c) => c === RNABase.CUT);
  }
   ...
   */
  public static fromSequenceString(
    seq: string,
    allowCut = true,
    allowUnknown = true
  ) {
  ...
  public getExpColoredSequence(expData: number[]): string {
    ...
    const offset: number = expData[0] as number;
    ...
    for (let ii = 0; ii < this.length; ii++) {
      if (ii < offset - 1 || ii >= expData.length) {
        res += str[ii];
      } else if ((expData[ii] as number) < avg) {
        res += `<FONT COLOR='#7777FF'>${str[ii] as string}</FONT>`;
      } else {
        res += `<FONT COLOR='#FF7777'>${str[ii] as string}</FONT>`;
      }
    }
   ...
   */
  public countConsecutive(
    letter: RNABase,
    locks: boolean[] | null = null
  ): number {
        ...
        if (maxConsecutive < ii - startIndex) {
          if (locks == null) {
            ...
            for (jj = startIndex; jj < ii; jj++) {
              allLocked = allLocked && Boolean(locks[jj]);
            }
            if (allLocked === false) {
              maxConsecutive = ii - startIndex;
            }
  ...
  public numGUPairs(pairs: SecStruct): number {
    ...
    for (let ii = 0; ii < pairs.length; ii++) {
      if (pairs.pairingPartner(ii) > ii) {
        if (
          this._baseArray[ii] === RNABase.GUANINE &&
          this._baseArray[pairs.pairingPartner(ii)] === RNABase.URACIL
        ) {
          ret++;
        }
        if (
          this._baseArray[ii] === RNABase.URACIL &&
  ...
  public numGCPairs(pairs: SecStruct): number {
    ...
    for (let ii = 0; ii < pairs.length; ii++) {
      if (pairs.pairingPartner(ii) > ii) {
        ...
        if (
          this._baseArray[ii] === RNABase.CYTOSINE &&
          this._baseArray[pairs.pairingPartner(ii)] === RNABase.GUANINE
        ) {
          ret++;
        }
  ...
  public numUAPairs(pairs: SecStruct): number {
    ...
    for (let ii = 0; ii < pairs.length; ii++) {
      if (pairs.pairingPartner(ii) > ii) {
        if (
          this._baseArray[ii] === RNABase.ADENINE &&
          this._baseArray[pairs.pairingPartner(ii)] === RNABase.URACIL
        ) {
          ret++;
        }
  ...
  public count(baseType: RNABase) {
    return this._baseArray.reduce(
      (acc, curr) => acc + (curr === baseType ? 1 : 0),
      0
    );
  }
  ...
  public nt(ii: number): RNABase {
    return this._baseArray[ii] as RNABase;
  }
  ...
  public sequenceString(allowCut = true, allowUnknown = true): string {
    return this._baseArray
      .map((value) => EPars.nucleotideToString(value, allowCut, allowUnknown))
      .join('');
  }
